...
export function getMillisecondsUntilNextMidnightForTimeZone(
  rawTimeZone: string | null | undefined,
  now = new Date(),
): number {
  const timeZone = normalizeLabTimeZone(rawTimeZone);
  const todayDateKey = formatDateKeyForTimeZone(now, timeZone);
  const tomorrowDateKey = addDaysToDateKey(todayDateKey, 1);
  const tomorrowParts = parseDateKey(tomorrowDateKey);

  if (!tomorrowParts) {
    return 0;
  }

  const nextMidnightUtc = zonedDateTimeToUtc(tomorrowParts, timeZone, 0, 0, 0, 0);
