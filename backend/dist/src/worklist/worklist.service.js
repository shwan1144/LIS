...
let WorklistService = WorklistService_1 = class WorklistService {
    ...
    async getWorklistOrders(labId, params, userId) {
        ...
        try {
            ...
            if (params.orderStatus === order_entity_1.OrderStatus.CANCELLED) {
            ...
            const items = rows.map((row) => {
                ...
                const orderStatus = row.orderStatus ?? order_entity_1.OrderStatus.REGISTERED;
                const isCancelledOrder = orderStatus === order_entity_1.OrderStatus.CANCELLED;
                return {
                    orderId: row.orderId,
    ...
    async batchEnterResults(labId, actor, actorRole, updates) {
        ...
        for (const data of updates) {
            ...
            if (orderTest.sample.order.status === order_entity_1.OrderStatus.CANCELLED) {
                continue;
            }
            const forceEditVerified = data.forceEditVerified === true;
    ...
    async verifyMultiple(orderTestIds, labId, actor) {
        ...
        for (const ot of orderTests) {
            if (ot.sample.order.labId !== labId ||
                ot.sample.order.status === order_entity_1.OrderStatus.CANCELLED ||
    ...
    assertOrderAllowsResultMutation(order) {
        if (order.status === order_entity_1.OrderStatus.CANCELLED) {
            throw new common_1.BadRequestException('Cancelled orders are read-only.');
        }
    }
    assertOrderAllowsResultRelease(order) {
        if (order.status === order_entity_1.OrderStatus.CANCELLED) {
