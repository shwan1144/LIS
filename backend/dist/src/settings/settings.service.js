...
let SettingsService = class SettingsService {
    ...
    async updateLabSettings(labId, data) {
        ...
        if (data.reportStyle !== undefined) {
            if (data.reportStyle === null) {
                ...
                try {
                    lab.reportStyle = (0, report_style_config_1.validateAndNormalizeReportStyleConfig)(data.reportStyle, 'reportStyle');
                }
                catch (error) {
    ...
    normalizePrintMethod(value) {
        ...
        if (value !== 'browser' && value !== 'direct_gateway') {
            throw new common_1.BadRequestException('printing.mode must be browser or direct_gateway');
        }
        return value;
    }
    ...
    async getUserWithDetails(id, labId) {
        ...
        return {
            ...
            shiftIds: user.shiftAssignments
    ...
    async updateUser(id, labId, data) {
        return this.userRepo.manager.transaction(async (manager) => {
            ...
            if (data.shiftIds !== undefined) {
                ...
                const shiftsInLab = await manager.getRepository(shift_entity_1.Shift).find({
                    where: { labId },
                    select: ['id'],
                });
            ...
            if (data.departmentIds !== undefined) {
                ...
                const departmentsInLab = await manager.getRepository(department_entity_1.Department).find({
                    where: { labId },
                    select: ['id'],
                });
