...
let ReportsService = ReportsService_1 = class ReportsService {
    ...
    async searchPublicResultHistory(input) {
        ...
        const maxRows = Number.isFinite(input.limit) ? Math.max(1, Math.min(50, Math.floor(input.limit))) : 50;
        ...
        for (const order of candidateOrders) {
            ...
            const verifiedCount = reportableOrderTests.filter((ot) => ot.status === 'VERIFIED' || !!ot.verifiedAt).length;
