...
const results_report_print_prep_1 = require("./html/results-report-print-prep");
...
let ReportsService = ReportsService_1 = class ReportsService {
    constructor(orderRepo, orderTestRepo, testComponentRepo, patientRepo, labRepo, userRepo, auditLogRepo, resultDocumentsService, fileStorageService) {
        ...
        this.reportStorageSyncInFlight = new Map();
    ...
    mapResultDocumentSummary(orderTest) {
    ...
    async renderPdfFromHtml(html) {
        ...
        try {
            ...
            await page.evaluate(results_report_print_prep_1.prepareResultsReportDocumentForPrint);
    ...
    buildStoredReportPdfObjectKey(input) {
        ...
        const fingerprint = (0, crypto_1.createHash)('sha1').update(rawKey).digest('hex');
        return `reports/${input.labId}/${input.order.id}/${fingerprint}.pdf`;
    }
    isReportReadyForStorage(reportableOrderTests, verifiedTests) {
    ...
    getPublicResultDepartmentName(orderTest) {
        const test = orderTest.test;
        return (test?.department?.name ||
            'General Department');
    }
    getPublicExpectedCompletionMinutes(orderTest, fallbackChildren = []) {
        const test = orderTest.test;
        ...
        const maxChildExpected = fallbackChildren.reduce((max, child) => {
            const childExpected = Number(child.test?.expectedCompletionMinutes ?? 0);
    ...
    buildPublicResultTestItem(orderTest, overrides = {}) {
        const test = orderTest.test;
        ...
        return {
            ...
            isVerified: orderTest.status === order_test_entity_1.OrderTestStatus.VERIFIED,
    ...
    resolveCollapsedPublicPanelStatus(panelItems) {
        if (panelItems.length === 0) {
            return order_test_entity_1.OrderTestStatus.PENDING;
        }
        if (panelItems.every((item) => item.status === order_test_entity_1.OrderTestStatus.VERIFIED)) {
            return order_test_entity_1.OrderTestStatus.VERIFIED;
        }
        if (panelItems.some((item) => item.status === order_test_entity_1.OrderTestStatus.REJECTED)) {
            return order_test_entity_1.OrderTestStatus.REJECTED;
        }
        if (panelItems.some((item) => item.status === order_test_entity_1.OrderTestStatus.IN_PROGRESS ||
            item.status === order_test_entity_1.OrderTestStatus.COMPLETED ||
            item.status === order_test_entity_1.OrderTestStatus.VERIFIED)) {
            return order_test_entity_1.OrderTestStatus.IN_PROGRESS;
        }
        return order_test_entity_1.OrderTestStatus.PENDING;
    }
    buildPublicResultDisplayTests(reportableOrderTests, options) {
        const { regularTests, panelParents, panelChildrenByParent } = this.classifyOrderTestsForReport(reportableOrderTests);
        const exposeResultDocuments = options?.exposeResultDocuments ?? true;
        const regularItems = regularTests.map((orderTest) => this.buildPublicResultTestItem(orderTest, exposeResultDocuments ? {} : { resultDocument: null }));
        const panelItems = panelParents.map((panelParent) => {
            ...
            const latestVerifiedAt = panelMembers
            ...
            return this.buildPublicResultTestItem(panelParent, {
                ...
                status: panelStatus,
                isVerified: panelStatus === order_test_entity_1.OrderTestStatus.VERIFIED,
                hasResult,
                resultValue: null,
    ...
    isPublicResultPaymentComplete(order) {
    ...
    async getPublicResultStatus(orderId) {
        ...
        const ready = canAccessUploadedDocuments &&
    ...
    async getPublicResultDocument(orderId, orderTestId) {
        ...
        if (!this.isPublicResultPaymentComplete(order)) {
            throw new common_1.ForbiddenException('Order is unpaid or partially paid. Complete payment to access uploaded PDF results.');
        }
    ...
    async buildResultsReportHtmlDocument(input) {
        const verifierLookupStartMs = Date.now();
        const verifierIds = [
            ...new Set(input.orderTests
        ...
        if (kurdishFontPath) {
            try {
                if (!ReportsService_1.cachedFont || ReportsService_1.cachedFont.path !== kurdishFontPath) {
                    ...
                    ReportsService_1.cachedFont = {
                        path: kurdishFontPath,
                        base64: `data:font/ttf;base64,${fontBuf.toString('base64')}`,
                    };
                }
        ...
        const assetsMs = Date.now() - assetsStartMs;
        const orderQrDataUrl = await this.generateOrderQrDataUrl(input.order);
        const htmlStartMs = Date.now();
        const html = (0, results_report_template_1.buildResultsReportHtml)({
            order: input.order,
            orderTests: input.orderTests,
            verifiedCount: input.verifiedTests.length,
            reportableCount: input.orderTests.length,
            verifiers: verifierNames,
            latestVerifiedAt: input.latestVerifiedAt ?? null,
            comments,
            kurdishFontBase64,
            orderQrDataUrl,
        });
        const htmlMs = Date.now() - htmlStartMs;
        return {
            html,
            verifiers: verifierNames,
            comments,
    ...
    async generateTestResultsPrintHtml(orderId, labId, options) {
        ...
        const renderedOrderTestsWithSections = this.attachPanelSectionMetadata(reportableOrderTests, panelSectionLookup.byPanelAndChildTest);
        ...
        if (this.getPdfReportAttachments(renderedOrderTestsWithSections).length > 0) {
            throw new common_1.ConflictException({
                code: 'REPORT_HTML_PRINT_REQUIRES_PDF',
                message: 'This report includes attached PDF result pages. Standard PDF print is required so attachments are included.',
            });
        }
        const { html } = await this.buildResultsReportHtmlDocument({
            order: orderForRender,
            orderTests: renderedOrderTestsWithSections,
            verifiedTests,
            latestVerifiedAt: latestVerifiedAt ?? null,
        });
        return (0, results_report_print_prep_1.injectResultsReportPrintPreparationScript)(html);
    }
    async syncReportToS3(orderId, labId) {
        ...
        const syncPromise = (async () => {
            try {
                ...
                const panelSectionLookup = await this.loadPanelSectionLookup(reportableOrderTests);
                const reportableOrderTestsWithSections = this.attachPanelSectionMetadata(reportableOrderTests, panelSectionLookup.byPanelAndChildTest);
                ...
                if (order.reportS3Key === expectedKey && order.reportGeneratedAt) {
                    try {
                        ...
                        this.logger.warn(`Stored report object missing for order ${orderId} (${expectedKey}); regenerating: ${message}`);
                    }
                }
    ...
    async generateTestResultsPDFWithProfile(orderId, labId, options) {
        ...
        const generatePdf = async () => {
            const htmlDocument = await this.buildResultsReportHtmlDocument({
                order: orderForRender,
                orderTests: renderedOrderTestsWithSections,
                verifiedTests: renderedVerifiedTests,
                latestVerifiedAt: latestVerifiedAt ?? null,
            });
            verifierLookupMs = htmlDocument.verifierLookupMs;
            assetsMs = htmlDocument.assetsMs;
            htmlMs = htmlDocument.htmlMs;
            try {
                const renderStartMs = Date.now();
                const pdf = await this.renderPdfFromHtml(htmlDocument.html);
                ...
                const fallbackPdf = await this.renderTestResultsFallbackPDF({
                    order: orderForRender,
                    orderTests: renderedOrderTestsWithSections,
                    verifiers: htmlDocument.verifiers,
                    latestVerifiedAt: latestVerifiedAt ?? null,
                    comments: htmlDocument.comments,
                });
    ...
    async renderTestResultsFallbackPDF(input) {
        ...
        const reportStyle = (0, report_style_config_1.resolveReportStyleConfig)(labBranding?.reportStyle);
        ...
        return new Promise((resolve, reject) => {
            ...
            const buildPanelFallbackLayout = (options) => {
                const baseWeights = options.isParameterTable
                    ? (showStatusColumn
            ...
            const drawPanelTableHeader = (layout) => {
                let cursorX = leftX;
                doc.font('Helvetica-Bold').fontSize(10).fillColor('#111827');
            ...
            const drawPanelResultRow = (layout, row) => {
            ...
            const drawPanelOrderTestRow = (layout, ot) => {
                const t = ot.test;
                const flag = (0, order_test_flag_util_1.normalizeOrderTestFlag)(ot.flag ?? null) ?? String(ot.flag ?? '').trim().toUpperCase();
            ...
            for (let panelIndex = 0; panelIndex < panelParents.length; panelIndex++) {
                ...
                const parameterDefinitions = Array.isArray(panelTest?.parameterDefinitions)
                    ...
                    : [];
                const isParameterTable = parameterDefinitions.length > 0 || Object.keys(panelResultParams).length > 0;
