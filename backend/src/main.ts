...
function resolveApiBodyLimit(): string {
  ...
  if (!/^\d+(b|kb|mb|gb)$/i.test(configured)) {
    bootstrapLogger.warn(
      `Invalid API_BODY_LIMIT="${configured}". Falling back to ${DEFAULT_API_BODY_LIMIT}.`,
    );
    return DEFAULT_API_BODY_LIMIT;
  }
...
async function ensureReportBrandingColumns(dataSource: DataSource): Promise<void> {
  const sqlStatements = [
    ...
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentStorageKey" text',
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentFileName" varchar(255)',
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentMimeType" varchar(100)',
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentSizeBytes" integer',
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentUploadedAt" timestamp',
    'ALTER TABLE IF EXISTS "order_tests" ADD COLUMN IF NOT EXISTS "resultDocumentUploadedBy" uuid',
    ...
    'ALTER TABLE IF EXISTS "tests" ADD COLUMN IF NOT EXISTS "showPanelUnitColumnInReport" boolean NOT NULL DEFAULT true',
    `UPDATE "tests"
    ...
    `UPDATE "tests"
      SET "showPanelUnitColumnInReport" = true
      WHERE "showPanelUnitColumnInReport" IS NULL`,
...
async function ensureGatewayDedupSchema(dataSource: DataSource): Promise<void> {
  const sqlStatements = [
...
async function bootstrap() {
  ...
  app.use(json({ limit: apiBodyLimit }));
  app.use(urlencoded({ extended: true, limit: apiBodyLimit }));
  ...
  try {
    ...
    bootstrapLogger.warn(
      `Failed to auto-ensure gateway dedup schema: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  try {
