...
function getLoadingMessage(status: PublicResultStatus): string {
  if (status.ready) {
...
function getTestStatusMeta(test: PublicResultTestItem): { label: string; className: string } {
  if (test.isVerified) {
...
function renderTestsSection(status: PublicResultStatus): string {
  if (status.tests.length === 0) {
    return '<div class="hint">No reportable tests found.</div>';
  }

  const groups = new Map<string, PublicResultTestItem[]>();
