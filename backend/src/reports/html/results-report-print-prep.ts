export const RESULTS_REPORT_PRINT_READY_FLAG = '__lisResultsPrintReady';
export const RESULTS_REPORT_PRINT_ERROR_FLAG = '__lisResultsPrintError';
export const RESULTS_REPORT_PRINT_READY_EVENT = 'lis-results-print-ready';
export const RESULTS_REPORT_PRINT_ERROR_EVENT = 'lis-results-print-error';

export async function prepareResultsReportDocumentForPrint(): Promise<void> {
  const fontReady = (document as Document & { fonts?: { ready?: Promise<unknown> } }).fonts?.ready;
  if (fontReady) {
    try {
      await fontReady;
    } catch {
      // Ignore font-loading failures and continue with available fonts.
    }
  }
  ...
  const footer = document.querySelector('.report-footer') as HTMLElement | null;
  if (footer) {
...
export function injectResultsReportPrintPreparationScript(html: string): string {
  const script = `
<script>
(function () {
  var READY_FLAG = ${JSON.stringify(RESULTS_REPORT_PRINT_READY_FLAG)};
  var ERROR_FLAG = ${JSON.stringify(RESULTS_REPORT_PRINT_ERROR_FLAG)};
  var READY_EVENT = ${JSON.stringify(RESULTS_REPORT_PRINT_READY_EVENT)};
  var ERROR_EVENT = ${JSON.stringify(RESULTS_REPORT_PRINT_ERROR_EVENT)};
