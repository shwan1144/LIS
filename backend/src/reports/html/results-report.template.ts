...
function formatParams(params: Record<string, string> | null): string[] {
  ...
  return Object.entries(params)
    .filter(([k, v]) => k !== '__cultureResult' && v != null && String(v).trim() !== '')
...
function isCultureTest(ot: OrderTest): boolean {
  const entryType = ((ot.test as any)?.resultEntryType || '').toUpperCase();
  return entryType === 'CULTURE_SENSITIVITY';
}

function buildCultureHtml(ot: OrderTest): string {
  const resultParams = ot.resultParameters && typeof ot.resultParameters === 'object' ? ot.resultParameters : {};
  const raw = resultParams['__cultureResult'];
  if (!raw) {
    // Fallback: just show resultText
    return `<div style="padding:10px;">${escapeHtml(ot.resultText || 'No result')}</div>`;
  }
  let cultureData: { noGrowth?: boolean; organisms?: any[]; comments?: string };
  try {
    cultureData = JSON.parse(raw);
  } catch {
    return `<div style="padding:10px;">${escapeHtml(ot.resultText || 'No result')}</div>`;
  }

  if (cultureData.noGrowth) {
    let html = '<div style="padding:12px 0;font-size:14px;font-weight:700;color:#0f8a1f;">No Growth After 48 Hours of Incubation</div>';
    if (cultureData.comments) {
      html += `<div style="margin-top:6px;font-size:11px;"><strong>Comments:</strong> ${escapeHtml(cultureData.comments)}</div>`;
    }
    return html;
  }
  ...
  for (let i = 0; i < organisms.length; i++) {
    ...
    html += `<div style="font-size:13px;font-weight:700;margin-bottom:6px;border-bottom:1px solid #ccc;padding-bottom:4px;">Organism ${i + 1}: <em>${escapeHtml(org.name || '-')}</em> &mdash; Colony Count: ${escapeHtml(org.quantity || '-')}</div>`;

    const sensitivities = org.sensitivities || [];
    if (sensitivities.length > 0) {
      html += `<table style="width:100%;border-collapse:collapse;margin-top:4px;">`;
      html += `<thead><tr>
        <th style="border:1px solid #ddd;padding:5px 8px;text-align:left;background:#f2f2f2;width:40%;">Antibiotic</th>
        <th style="border:1px solid #ddd;padding:5px 8px;text-align:center;background:#f6ffed;width:20%;color:#0f8a1f;">Susceptible (S)</th>
        <th style="border:1px solid #ddd;padding:5px 8px;text-align:center;background:#fffbe6;width:20%;color:#d48806;">Intermediate (I)</th>
        <th style="border:1px solid #ddd;padding:5px 8px;text-align:center;background:#fff2f0;width:20%;color:#cf1322;">Resistant (R)</th>
      </tr></thead><tbody>`;
      for (const s of sensitivities) {
        const sCheck = s.result === 'S' ? '&#10004;' : '';
        const iCheck = s.result === 'I' ? '&#10004;' : '';
        const rCheck = s.result === 'R' ? '&#10004;' : '';
        html += `<tr>
          <td style="border:1px solid #eee;padding:4px 8px;font-weight:600;">${escapeHtml(s.antibiotic || '-')}</td>
          <td style="border:1px solid #eee;padding:4px 8px;text-align:center;font-size:14px;${s.result === 'S' ? 'color:#0f8a1f;font-weight:700;' : ''}">${sCheck}</td>
          <td style="border:1px solid #eee;padding:4px 8px;text-align:center;font-size:14px;${s.result === 'I' ? 'color:#d48806;font-weight:700;' : ''}">${iCheck}</td>
          <td style="border:1px solid #eee;padding:4px 8px;text-align:center;font-size:14px;${s.result === 'R' ? 'color:#cf1322;font-weight:700;' : ''}">${rCheck}</td>
        </tr>`;
      }
      html += `</tbody></table>`;
    } else {
      html += `<div style="font-size:11px;color:#999;margin-top:4px;">No antibiotic sensitivity data</div>`;
    }
    html += `</div>`;
  }

  if (cultureData.comments) {
    html += `<div style="margin-top:8px;font-size:11px;"><strong>Comments:</strong> ${escapeHtml(cultureData.comments)}</div>`;
  }
...
export function buildResultsReportHtml(input: {
  ...
  const kurdishFontFace = input.kurdishFontBase64
    ...
    : '';
  const templateLogo =
    'data:image/png;base64,أ¢â‚¬آ¦48787 tokens truncatedأ¢â‚¬آ¦';
  ...
  const commentsText = input.comments.length ? input.comments.join(' • ') : '';
  ...
  const regularTests = flattenedTests.filter(
    (ot) => ot.parentOrderTestId || ((ot.test as any)?.type !== TestType.PANEL && !isCultureTest(ot))
  );
  const cultureTests = flattenedTests.filter(
    (ot) => !ot.parentOrderTestId && isCultureTest(ot)
  );
  ...
  // Build culture content HTML
  let cultureContentHtml = '';
  for (const ot of cultureTests) {
    const t: any = ot.test;
    const testName = t?.name || t?.code || 'Culture & Sensitivity';
    cultureContentHtml += `
      <div class="panel-section" style="margin-top: 20px;">
        <div class="panel-page-title">${escapeHtml(testName)}</div>
        ${buildCultureHtml(ot)}
  ...
  if (!sectionsHtml && regularTests.length === 0 && panelTests.length === 0 && cultureTests.length === 0) {
  ...
  if (regularTests.length > 0) {
    ...
    <div class="page">
      ...
      <div class="content">
        ${sectionsHtml}
        ${commentsText && panelTests.length === 0 && cultureTests.length === 0 ? `<div class="comments" style="margin-top:8px;font-size:11px;font-weight:700;"><strong>Comments:</strong> ${escapeHtml(commentsText)}</div>` : ''}
      </div>
      ${pageFooterHtml}
    </div>`;
  }
  ...
  if (panelContentHtml) {
    ...
    <div class="page" style="page-break-before: ${regularTests.length > 0 ? 'always' : 'auto'};">
      ...
      <div class="content">
        ${panelContentHtml}
        ${commentsText && cultureTests.length === 0 ? `<div class="comments" style="margin-top:8px;font-size:11px;font-weight:700;"><strong>Comments:</strong> ${escapeHtml(commentsText)}</div>` : ''}
      </div>
      ${pageFooterHtml}
    </div>`;
  }

  // Page 3: Culture & Sensitivity tests (if any)
  if (cultureContentHtml) {
    pagesHtml += `
    <div class="page" style="page-break-before: ${regularTests.length > 0 || panelContentHtml ? 'always' : 'auto'};">
      ${pageHeaderHtml}
      <div class="content">
        ${cultureContentHtml}
  ...
  // If no tests at all
  if (regularTests.length === 0 && panelTests.length === 0 && cultureTests.length === 0) {
    pagesHtml += `
    <div class="page">
      ${pageHeaderHtml}
      <div class="content">
