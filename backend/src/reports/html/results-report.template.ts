...
function getPanelReportSection(ot: OrderTest): string | null {
  const raw = (ot as { panelReportSection?: unknown }).panelReportSection;
...
export function buildResultsReportHtml(input: {
  ...
  const resultsBodyFontFamily = resolveReportFontStackWithArabicFallback(
    reportStyle.resultsTable.bodyStyle.fontFamily,
  );
  const resultsDepartmentFontFamily = resolveReportFontStackWithArabicFallback(
    reportStyle.resultsTable.departmentRowStyle.fontFamily,
  );
  const resultsCategoryFontFamily = resolveReportFontStackWithArabicFallback(
    reportStyle.resultsTable.categoryRowStyle.fontFamily,
  );
  const cultureSectionFontFamily = resolveReportFontStackWithArabicFallback(
  ...
  // Render regular tests as a single table with one page-level header.
  // Department blocks are isolated in tbody sections so print pagination
  // can move whole departments to next page when possible.
